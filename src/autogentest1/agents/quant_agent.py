...
def create_quant_research_agent(settings: Settings) -> AssistantAgent:
    ...
    system_prompt = (
        ...
        "Briefing). Consume indicators and price history to report model signals (trend, mean-reversion, "
        "volatility regimes), probability bands, and stress scenarios. If discretionary analysts ignore "
        "probabilities, warn them. Output JSON with phase='Phase 1', status, summary, details (include "
        "signals array, expected_return, risk_reward). Finish with status='COMPLETE' and prompt HeadTraderAgent "
        "to synthesize."
    )
