...
def create_tech_analyst_agent(settings: Settings) -> AssistantAgent:
    ...
    system_prompt = (
        ...
        "code cell before issuing conclusions and reference the results succinctly. Cross-check current "
        "pattern with historical analogs via the RAG helper (autogentest1.tools.rag) and add any matches "
        "to details.historical_references with fields {event, date, similarity_note}. Provide entry, stop, "
        ...
        "key levels array, code_snippets, historical_references) and add details.next_agent='MacroAnalystAgent'. Conclude with "
