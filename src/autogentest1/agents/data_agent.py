...
def create_data_agent(settings: Settings) -> AssistantAgent:
    ...
    system_prompt = (
        ...
        "Python, capture returned JSON, and cite the source in your summary. Before finalising, query the "
        "RAG helper (autogentest1.tools.rag) for historical precedents similar to today's risk backdrop and "
        "store any matches inside details.historical_references (list of {event, year, takeaway}). Output strictly in JSON "
        ...
        "collecting, then 'COMPLETE' once numbers plus historical context are delivered. Include numerical fields (price, atr, "
