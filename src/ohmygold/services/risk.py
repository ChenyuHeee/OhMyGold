...
def _compute_liquidity_metrics(history: Any, latest_price: Optional[float]) -> Dict[str, Any]:
    ...
    spreads_bps_series: Optional["pd.Series"] = None
    ...
    if latest_price and "High" in history and "Low" in history and "Close" in history:
        ...
        try:
            ...
            close_nonzero = closes.replace(0, pd.NA)
            spreads_raw = ((highs - lows) / close_nonzero).dropna()
            if not spreads_raw.empty:
                spreads_bps_series = spreads_raw * 5_000
    ...
    if spreads_bps_series is not None and not spreads_bps_series.empty:
        spreads_clean = pd.Series(spreads_bps_series).dropna()
        if not spreads_clean.empty:
            metrics["spread_bps_samples"] = int(len(spreads_clean))
            metrics["spread_bps_avg"] = float(spreads_clean.mean())
            metrics["spread_bps_max"] = float(spreads_clean.max())
            try:
                metrics["spread_bps_p95"] = float(np.percentile(spreads_clean, 95))
            except Exception:  # pragma: no cover - percentile fallback
                metrics["spread_bps_p95"] = float(spreads_clean.quantile(0.95))
