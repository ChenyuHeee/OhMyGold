...
- 配置方式：在 `.env` 中设置 `LOCAL_MODEL_ENABLED=true`，填写 `LOCAL_MODEL_NAME`、`LOCAL_MODEL_BASE_URL`、`LOCAL_MODEL_AGENTS`（如 `DataAgent,MacroAnalystAgent,QuantResearchAgent`）。
- 运行逻辑：被列入 `LOCAL_MODEL_AGENTS` 的角色优先调用本地模型，失败时自动回退至 DeepSeek；其余角色仍直接使用 DeepSeek。
- 建议分工：让 Phase 1 的研究角色、Phase 4 风控初审等高频任务落在本地模型，关键决策仍交由云端模型兜底。*** End Patch
